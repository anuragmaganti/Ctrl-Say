...
- Temporary and permanent named copies both use normalized one-to-five-word names. `permanent copy <name>` stores its name in a protected namespace and persists locally across quits, restarts, rebuilds, and app updates until explicitly deleted; temporary commands cannot overwrite it and no permanent data syncs to the cloud.
